...
The App requests access to your device's **Photo Library** (`android.permission.READ_MEDIA_IMAGES` or iOS Photos permission).
* **Usage:** This permission is required solely to enable the "Gallery" activity, which allows children to view their *own* device photos within the App's interface.
* **Data Safety:** These photos are processed **locally on your device**. We do not collect, upload, share, or store your personal photos on our servers or any third-party servers.

#### Microphone Access (Audio Playback)
The App may request or list permissions related to the microphone (`android.permission.RECORD_AUDIO`).
* **Why this appears:** This permission is a technical requirement included by third-party audio libraries (such as `expo-audio`) used to play the high-quality voiceovers for our Alphabet and Numbers games.
* **Our Usage:** PlayAces **DOES NOT** use the microphone to record, collect, or listen to any audio from you or your child. The permission is strictly a byproduct of the sound playback engine.
...
* **Encryption:** Authentication tokens transmitted between the App and identity providers (Google/Apple) are encrypted using secure protocols (HTTPS/SSL).
* **Local Processing:** Critical features, such as the Photo Gallery, are designed to function locally, ensuring your personal media never leaves your device.
